...
my_uni_cph <- function(df, var, time_to_event, event, confounders=NULL, show_diff = FALSE, 
  ...
  if(!firth){
    ...
    if(show.confounders) {display(summary(mod), diff= show_diff, effect = effect)}else{
      resout <- display(summary(mod), diff= show_diff, effect = effect)
    ...
    if(show.confounders) {display(mod, diff= show_diff, effect = effect, firth = T)}else{
      resout <- display(mod, diff= show_diff, effect = effect, firth = T)
      idx <- grep(var, rownames(resout))
      resout <- resout[idx,]
      resout
    } 
